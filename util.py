...
FPATH = 'external_labels.csv'

# there needs to be a input file named 'external_labels.csv' with columns 'virushost', and 'label'

def load_external_labels(session, candidate_class, annotator_name='gold'):
    gold_labels = pd.read_csv(FPATH)
    for index, row in gold_labels.iterrows():    

        # We check if the label already exists, in case this cell was already executed
        context_stable_ids = row['virushost']
