...
for fold in range(5):
    ...
    # Optimal parameters from LSTM grid search notebook
    train_kwargs = {
        'lr':              0.001,
        'embedding_dim':   100,
        'hidden_dim':      100,
    ...
    lstm.train(train_cands, train_marginals, X_dev=dev_cands,
               Y_dev=L_gold_dev, **train_kwargs)

    p, r, f1 = lstm.score(
        X_test=test_cands, Y_test=L_gold_test, set_unlabeled_as_neg=False)
    ...
    tp, fp, tn, fn = lstm.error_analysis(
        session, test_cands, L_gold_test, set_unlabeled_as_neg=False)

    accuracy = (len(tp)+len(tn))/(len(tp)+len(tn)+len(fp)+len(tn))
    accs.append(accuracy)

    # save marginals (raw predictions on candidates)
    test_marginals = lstm.marginals(X=test_cands)
...
plt.savefig("5_fold_cv_prc_LSTM.png")
plt.show()
...
df2.to_csv("LSTM_CV_results.csv", header=True)
