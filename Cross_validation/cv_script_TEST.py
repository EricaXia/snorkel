...
for fold in range(5):
    ...
    logreg.train(X_train = F_train, Y_train = train_marginals, n_epochs = 30, lr = 1.00e-02, batch_size = 64, rebalance = 5.00e-01, X_dev = F_dev, Y_dev = L_gold_dev, print_freq = 10, dev_ckpt_delay = 0.5, l1_penalty = 1.00e-06, l2_penalty = 1.00e-06)
        
    p, r, f1 = logreg.score(X_test = F_test, Y_test = L_gold_test, set_unlabeled_as_neg=False)
    print("Prec: {0:.3f}, Recall: {1:.3f}, F1 Score: {2:.3f}".format(p, r, f1))
    precs.append(p) # save for later
    recalls.append(r)
    f1s.append(f1)
    
    tp, fp, tn, fn = logreg.error_analysis(session, F_test, L_gold_test, set_unlabeled_as_neg = False)
...
# # show final plot
# plt.title("3-Fold CV Precision Recall Curve")
# plt.xlabel('Recall')
# plt.ylabel('Precision')
# plt.legend(loc="lower right")
# plt.savefig("3_fold_cv_prc.png")
# plt.show()
# =============================================================================
