# -*- coding: utf-8 -*-
"""
Created on Wed Aug  7 12:08:40 2019
...
ictv_viruses['Species2'] = ictv_viruses['Species'].str.replace(
    '\d+', '', regex=True)
...
dont_want = ['Mal', 'mal', 'Ou', 'ou', 'Marta',
             'marta', 'León', 'león', 'Euro', 'euro']

for word in dont_want:
    if word in animals_list:
        animals_list.remove(word)

dont_want2 = ['bat', 'BAT', 'langur', 'mcp', 'MCP', 'con', 'CON', 'spf', 'SPF', '(SPF)', 'his', 'His', 'HIS', 'pfu', 'PFU', '(PFU)', '(NSP)',
              'mal', 'MAL', 'Mal', 'ifa', 'IFA', '(IFA)', 'wrc', 'WRC', '(WRC)', 'fitc', '(fitc)', 'fam', 'Cherry', 'cherry', 'ihc', 'IHC', '(IHC)', 'hit']
...
detect = {'detect', 'detects', 'detected',
          'detecting', 'detection', 'detectable'}
detect_l = ['detect', 'detects', 'detected',
            'detecting', 'detection', 'detectable']
...
causal = ['caus(es|ed|e|ing|ation)?', 'induc(es|ed|e|ing)?',
          'associat(ed|ing|es|e|ion)?']

positive = {'detect', 'detects', 'detected', 'detecting', 'detection', 'detectable', 'infect', 'infects',
            'infected', 'infecting', 'infection', 'isolate', 'isolates', 'isolated', 'isolating', 'isolation'}
positive_l = ['detect', 'detects', 'detected', 'detecting', 'detection', 'detectable', 'infect', 'infects',
              'infected', 'infecting', 'infection', 'isolate', 'isolates', 'isolated', 'isolating', 'isolation']
...
def LF_positive2(c):
    return 1 if (
        re.search(r'{{A}}.{0,100} ' + ltp(positive_l) +
                  '.{0,100}{{B}}', get_tagged_text(c), re.I)
...
for fold in range(5):
    ...
    X_train, X_test = train_test_split(
        index_array, test_size=0.2, random_state=None)
    X_train, X_dev = train_test_split(
        X_train, test_size=0.2, random_state=None)
    ...
    # Load gold labels
    missed = load_external_labels(
        session, VirusHost, annotator_name='gold', split=1)
    L_gold_dev = load_gold_labels(session, annotator_name='gold', split=1)
    missed = load_external_labels(
        session, VirusHost, annotator_name='gold', split=2)
    ...
    gen_model.train(L_train, epochs=100, decay=0.95, step_size=0.1 /
                    L_train.shape[0], reg_param=1.00e-03, deps=deps)
    ...
    logreg = SparseLogisticRegression(n_threads=1)

    logreg.train(X_train=F_train, Y_train=train_marginals, n_epochs=30, lr=1.00e-02, batch_size=64, rebalance=5.00e-01,
                 X_dev=F_dev, Y_dev=L_gold_dev, print_freq=10, dev_ckpt_delay=0.5, l1_penalty=1.00e-06, l2_penalty=1.00e-06)

    p, r, f1 = logreg.score(
        X_test=F_test, Y_test=L_gold_test, set_unlabeled_as_neg=False)
    ...
    tp, fp, tn, fn = logreg.error_analysis(
        session, F_test, L_gold_test, set_unlabeled_as_neg=False)

    accuracy = (len(tp)+len(tn))/(len(tp)+len(tn)+len(fp)+len(tn))
    accs.append(accuracy)

    # save marginals (raw predictions on candidates)
    test_marginals = logreg.marginals(X=F_test)
    ...
    precision, recall, thresholds = precision_recall_curve(
        new_y, new_test_marginals, pos_label=1)

    # plot the current precision-recall curve
    plt.plot(recall, precision, marker='.', label='Fold %d' % (fold))
...
plt.savefig("5_fold_cv_prc_LOGREG.png")
plt.show()
...
df2.to_csv("LOGREG_CV_results.csv", header=True)
...
#     plt.plot(fpr, tpr, lw=1, alpha=0.3, label='ROC fold %d (AUC = %0.2f)' % ((fold+1), roc_auc))

#  # AFTER LOOP:
#  # plot the rest of the ROC curve
...
# plt.title('5-Fold CV Receiver operating characteristic')
# plt.legend(loc="lower right")
